...
if __name__ == '__main__':
    ...
    while True:
        ...
        print('acc: ', L[0][2])
        for j in range(5):
            mid = (a[L[j][0]][0] + a[L[j][0]][1]) / 2
            space[L[j][0]]['hy_lr'] = hp.uniform('hy_lr', a[L[j][0]][0], mid)
            space[L[9 - j][0]]['hy_lr'] = hp.uniform('hy_lr', mid, a[L[j][0]][1])
            a[L[9 - j][0]][0] = mid
            a[L[9 - j][0]][1] = a[L[j][0]][1]
            a[L[j][0]][1] = mid
            b[L[j][0]] = 0
            b[L[9 - j][0]] = 0
            trials = Trials()
            trials1 = Trials()
            trials2 = Trials()
            trials = pickler.load(open('trials'+ str(L[j][0]), 'rb'))
            for trial in trials:
                if trial['misc']['vals']['hy_lr'][0] < mid:
                    ...
                    trials2.refresh()
                    b[L[9 - j][0]] += 1
            pickler.dump(trials1, open('trials' + str(L[j][0]), 'wb'))
            pickler.dump(trials2, open('trials' + str(L[9 - j][0]), 'wb'))
        if L[0][2] <= -0.985:
            break
    e = time.time()
    print('time: ', e - s)
